...
def compute_node_centrality(graph):
    ...
    w_degrees = [round(float(degree) / (len(graph.vs) - 1), 5) for degree in w_degrees] #normalisation
...
def print_top10(feature_names, clf, class_labels):
    ...
    #                 #
    ###################
    for i, class_labels in enumerate(class_labels):
        top10 = np.argsort(clf.coef_[i])[-1][-10:]
        print("%s" %(class_labels, "".join(top10)))
...
def print_bot10(feature_names, clf, class_labels):
    ...
    ###################
    for i, class_labels in enumerate(class_labels):
        top10 = np.argsort(clf.coef_[i])[-1][0:10]

...
# 'my_path' should be the absolute path poiting to where the data files are located on your machine
...
my_pca = PCA(n_components=2) # fill
wv_2d = my_pca.fit_transform(word_vectors) # fill
# check
wv_2d.shape
...
# to fill: repeat line above for s_2
...
for fold in range(n_folds):
    ...
    for instance in instances_test:
        predictions_wmd_fold.append(# to fill (use the neighbors_predict function with the right arguments)) 
        predictions_tfidf_fold.append(# to fill (use the neighbors_predict function with the right arguments))
        print kkk
        kkk+=1
